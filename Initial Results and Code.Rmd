...
The attributes selected by chi-squared were the same as those that were found using information gain. CFS had attributes that were found in chi-squared. Therefore, attributes found with chi-squared and consistency method will be used to for model and compared to see which model proves to be stronger after evaluation. 

Logistic Regression Model predicting whether or not an individual has been diganosed by medical professional using chi-squared attributes: 

```{r}
analysis_train <- sample(nrow(analysis), floor(nrow(analysis)*0.7))
train <- clean[analysis_train,]
test <- clean[-analysis_train,]
model_cs_diag <- glm(formula = diagnosed_medpro ~ mh_past + sought_treatment +  mhi_work_treated_effectively + mhi_work_not_treated_effectively + mh_current, family = "binomial", data = train)
pred1 <- predict(model_cs_diag, interval = "prediction", newdata = test)
```

Logistic Regression Model for whether or not an individual sought treatment using chi-squared attributes:

```{r}
model_cs_treat <- glm(formula = sought_treatment ~ diagnosed_medpro + mh_past + mhi_work_treated_effectively + mhi_work_not_treated_effectively + mh_current, family = "binomial", data = train)
pred2 <- predict(model_cs_treat, interval = "prediction", newdata = test)
```

Logistic Regression Model predicting whether or not an individual has been diagnosed by medical professional using consistency attributes: 

```{r}
model_con_diag <- glm(formula = diagnosed_medpro ~ no_of_employees + mental_health_benefits + options_mental_health + mh_leave_request + pe_mh_benefits + team_neg_view_mh + mh_past + mh_current + sought_treatment + mhi_work_treated_effectively, family = "binomial", data = train)
pred3 <- predict(model_con_diag, interval = "prediction", newdata = test)
```

Logistic Regression Model predicting whether or not an individual sought treatment using consistency attributes: 

```{r}
model_con_treat <- glm(formula = sought_treatment ~ no_of_employees + mental_health_benefits + options_mental_health + anonymity + mh_leave_request + pe_mh_discuss_coworker + friends_family + mh_past + diagnosed_medpro + mhi_work_treated_effectively, family = "binomial", data = train)
pred4 <- predict(model_con_treat, interval = "prediction", newdata = test)
```
